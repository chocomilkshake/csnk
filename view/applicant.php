  ...
  <div class="offcanvas-lg offcanvas-start" tabindex="-1" id="filtersCanvas" aria-labelledby="filtersCanvasLabel">
    ...
    <div class="offcanvas-body">
      <form id="filtersForm" class="d-grid gap-3">
        ...
        <div class="filter-group">
          <div class="filter-group-title">
            ...
            <a href="#" class="filter-clear" id="clearAll">Clear All</a>
          </div>
          <div id="activeChips" class="active-filters" aria-live="polite" aria-label="Active filters">
            <!-- chips will appear here -->
          </div>
        </div>

        <!-- Specialization -->
        <div class="filter-group" data-group="specializations">
          <div class="filter-group-title">
            <span>Specialization <span class="filter-count" id="countSpecs">· 0</span></span>
            <a href="#" class="filter-clear" id="clearSpecs">Clear</a>
          </div>
        ...
        <!-- Availability -->
        <div class="filter-group" data-group="availability">
          <div class="filter-group-title">
            <span>Availability <span class="filter-count" id="countAvail">· 0</span></span>
            <a href="#" class="filter-clear" id="clearAvail">Clear</a>
          </div>
          <div class="vstack gap-1">
        ...
        <!-- Experience -->
        <div class="filter-group" data-group="experience">
          <div class="filter-group-title">
            <span>Experience</span>
            <a href="#" class="filter-clear" id="clearExp">Reset</a>
          </div>
          <label class="form-label small mb-1" for="exp-range">Minimum years <span id="expOut" class="filter-range-output">0</span></label>
          <input type="range" class="form-range" id="exp-range" name="min_experience" min="0" max="20" step="1" value="0">
        </div>

        <!-- Languages -->
        <div class="filter-group" data-group="languages">
          <div class="filter-group-title">
            <span>Languages <span class="filter-count" id="countLangs">· 0</span></span>
            <a href="#" class="filter-clear" id="clearLangs">Clear</a>
          </div>
          <div class="vstack gap-1">
        ...
        <!-- Sort -->
        <div class="filter-group">
          <div class="filter-group-title"><span>Sort by</span></div>
      ...
      <style>
        ...
        #filtersCanvas .offcanvas-body { padding: 1.25rem 1.2rem; }
        ...
        .filter-clear:hover { color: #991b1b; text-decoration: underline; }

        /* Selection count */
        .filter-count { font-weight: 700; color: #6c757d; }
        ...
        .filter-group .form-check:hover { background: #f8f9fa; }
        ...
        /* Buttons */
        #applyFilters { background: #991b1b; border-color: #991b1b; }
        #applyFilters:hover { background: #7a1515; border-color: #7a1515; }
...
<script>
      document.addEventListener('DOMContentLoaded', () => {
        ...
        const countSpecs = document.getElementById('countSpecs');
        const countAvail = document.getElementById('countAvail');
        const countLangs = document.getElementById('countLangs');
        const expRange = document.getElementById('exp-range');
        const expOut = document.getElementById('expOut');

        // Clear buttons
        const clearAll = document.getElementById('clearAll');
        const clearSpecs = document.getElementById('clearSpecs');
        const clearAvail = document.getElementById('clearAvail');
        const clearLangs = document.getElementById('clearLangs');
        const clearExp = document.getElementById('clearExp');

        // Helpers
        const $$ = (selector, root = form) => Array.from(root.querySelectorAll(selector));

        function getChecked(name) {
          return $$(`input[name="${name}[]"]:checked`).map(el => ({ id: el.id, value: el.value }));
        }

        function updateCounts() {
          if (countSpecs) countSpecs.textContent = `· ${getChecked('specializations').length}`;
          if (countAvail) countAvail.textContent = `· ${getChecked('availability').length}`;
          if (countLangs) countLangs.textContent = `· ${getChecked('languages').length}`;
        }

        function makeChip(text, onRemove) {
          const chip = document.createElement('span');
          chip.className = 'filter-chip';
          chip.innerHTML = `<span>${text}</span><button class="remove" type="button" aria-label="Remove">×</button>`;
        ...
        function renderChips() {
          ...
          // Specializations
          getChecked('specializations').forEach(({ id, value }) => {
            chipsWrap.appendChild(makeChip(value, () => {
          ...
          // Availability
          getChecked('availability').forEach(({ id, value }) => {
            chipsWrap.appendChild(makeChip(value, () => {
              document.getElementById(id)?.click();
              renderAll();
            }));
          ...
          // Languages
          getChecked('languages').forEach(({ id, value }) => {
            chipsWrap.appendChild(makeChip(value, () => {
              document.getElementById(id)?.click();
              renderAll();
            }));
          ...
          // Experience min
          if (expRange && Number(expRange.value) > 0) {
            chipsWrap.appendChild(makeChip(`Min Exp: ${expRange.value}y`, () => {
              expRange.value = 0;
              syncExp();
              renderAll();
            }));
        ...
        function syncExp() {
          if (expOut && expRange) expOut.textContent = expRange.value;
        }

        function clearGroup(name) {
          $$(`input[name="${name}[]"]:checked`).forEach(el => el.checked = false);
        }

        // Clear actions
        clearAll?.addEventListener('click', (e) => {
          e.preventDefault();
          clearGroup('specializations');
          clearGroup('availability');
          clearGroup('languages');
          if (expRange) { expRange.value = 0; syncExp(); }
          renderAll();
        });

        clearSpecs?.addEventListener('click', (e) => { e.preventDefault(); clearGroup('specializations'); renderAll(); });
        clearAvail?.addEventListener('click', (e) => { e.preventDefault(); clearGroup('availability'); renderAll(); });
        clearLangs?.addEventListener('click', (e) => { e.preventDefault(); clearGroup('languages'); renderAll(); });
        clearExp?.addEventListener('click', (e) => { e.preventDefault(); if (expRange) { expRange.value = 0; syncExp(); renderAll(); } });

        // Bind changes
        form.addEventListener('change', () => renderAll());
        expRange?.addEventListener('input', () => { syncExp(); renderAll(); });

        // Apply/Reset hook (optional UX)
        document.getElementById('applyFilters')?.addEventListener('click', (e) => {
          // If you submit via JS elsewhere, keep this. Otherwise remove preventDefault.
          // e.preventDefault();
          // Close offcanvas on mobile
          const oc = document.getElementById('filtersCanvas');
          if (oc?.classList.contains('show') && window.bootstrap) {
