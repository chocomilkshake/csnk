...
function fail(int $code, array $payload)
{
...
{
    if (!isset($src[$key]))
        fail(422, ['ok' => false, 'error' => "Missing field: $key"]);
...
try {
    ...
    if (!is_array($data))
        fail(400, ['ok' => false, 'error' => 'Invalid JSON']);
    ...
    // Verify the applicant belongs to Turkey business unit
    $appStmt = $db->prepare("SELECT business_unit_id FROM applicants WHERE id = ? AND deleted_at IS NULL");
    if (!$appStmt)
        fail(500, ['ok' => false, 'error' => 'Prepare failed (app): ' . $db->error]);
    ...
    // Use Turkey business unit ID (2)
    $business_unit_id = TURKEY_BUSINESS_UNIT_ID;
    ...
    $stmt = $db->prepare($sql);
    if (!$stmt)
        fail(500, ['ok' => false, 'error' => 'Prepare failed: ' . $db->error]);
    ...
    // Auto-mark applicant as ON PROCESS
    $statusUpdated = false;
