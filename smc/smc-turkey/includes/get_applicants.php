<?php
/**
 * Public API: Get Applicants for SMC Turkey (client-facing)
 * - CORS enabled
 * - Returns normalized, client-friendly fields
 * - Filters by business_unit_id = 2 (Turkey)
...
// Turkey Business Unit ID
define('TURKEY_BUSINESS_UNIT_ID', 2);
...
try {
    ...
    // Availability from UI (Full-time / Part-time)
    ...
    // Filter by Turkey Business Unit
    $where[] = "business_unit_id = " . TURKEY_BUSINESS_UNIT_ID;
    ...
    // Availability => employment_type tolerance (Full Time / Part Time)
    ...
    // Base URLs for Turkey (smc-turkey)
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
    // Adjust this path to your project root for Turkey
    $appBase = $scheme . '://' . $host . '/csnk/smc/smc-turkey';
