// app.js — Client listing UX (photo-top modern cards) for SMC Turkey
// v1.0 — Based on csnk version, adapted for Turkey

console.log('smc-turkey app.js loaded successfully - v1.0');
...
// ---- CONFIG: SMC Turkey app root path ----
const APP_BASE = '/csnk/smc/smc-turkey';
...
function setAvatar(imgEl, src, placeholder) {
  if (!imgEl) return;
  const fallback = placeholder || '../resources/img/placeholder-user.svg';
...
/* =========================================================
  Data loading (server-driven) - SMC TURKEY API
========================================================= */
let serverTotal = 0;
  ...
  const html = `
    <article class="card app-card h-100 hover-lift clickable-card" role="button" tabindex="0" aria-label="View ${fullName} profile" data-status="${status}">
      ...
      <div class="card-body">
        ...
        <div class="text-muted small mb-2 text-truncate">
          <i class="bi bi-geo-alt text-navy me-1"></i>${location}
        </div>
      ...
      <div class="card-footer bg-white">
        <button class="btn btn-outline-navy w-100 view-profile-btn" data-applicant-id="${applicant.id}">
...
function injectStyles(){
  if (document.getElementById('app-modern-card-styles-turkey')) return;
  const css = `
    :root{
      --smc-navy: #0B1F3A;
      --smc-navy-2: #132A4A;
  ...
  style.id = 'app-modern-card-styles-turkey';
...
function ensureVideoPopoverStyles(){
  if (document.getElementById('video-popover-styles-turkey')) return;
  const css = `
  .video-popover{
  ...
  `;
  const style = document.createElement('style');
  style.id = 'video-popover-styles-turkey';
  style.appendChild(document.createTextNode(css));
  document.head.appendChild(style);
}
