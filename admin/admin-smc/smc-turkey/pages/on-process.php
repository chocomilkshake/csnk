<?php
// FILE: admin/admin-smc/smc-turkey/pages/on-process.php (SMC - Turkey)
// Purpose: SMC-scoped "On Process" applicants with latest booking per applicant (BU-aware)

$pageTitle = 'On Process Applicants (SMC)';
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($conn instanceof mysqli) {
        // Ensure the applicant belongs to current BU (safety)
        $stmt = $conn->prepare("SELECT id FROM applicants WHERE id=? AND business_unit_id=? LIMIT 1");
        if ($stmt) {
            ...
            if ($row) {
                ...
                $stmt2 = $conn->prepare("UPDATE applicants SET deleted_at = NOW() WHERE id=?");
                if ($stmt2) {
    ...
    if ($deleted && isset($auth) && isset($_SESSION['admin_id']) && method_exists($auth, 'logActivity')) {
        ...
        $auth->logActivity((int)$_SESSION['admin_id'], 'Delete Applicant', "Deleted applicant {$label} (On Process list, SMC)");
    }
...
/* =========================================================
 * Status update WITH REPORT (POST) — SMC (MySQLi + BU id)
 * =========================================================*/
if (
    ...
    if (!isset($_POST['csrf_token'], $_SESSION['csrf_token']) ||
        ...
        if (function_exists('setFlashMessage')) setFlashMessage('error', 'Invalid or missing security token. Please refresh and try again.');
        redirect('on-process.php' . $qs);
        exit;
    }
    ...
    // Fetch applicant (verify BU & current status)
    $fromStatus = null;
    $fullName   = null;
    $stmt = $conn->prepare("SELECT status, first_name, middle_name, last_name, suffix FROM applicants WHERE id=? AND business_unit_id=? LIMIT 1");
    if ($stmt) {
        $stmt->bind_param("ii", $id, $currentBuId);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    if ($fromStatus === null || $fromStatus === '') {
        if (function_exists('setFlashMessage')) setFlashMessage('error', 'Applicant not found or not in your SMC BU.');
        redirect('on-process.php' . $qs);
        exit;
    }
    ...
    try {
        ...
        // IMPORTANT: SMC schema requires business_unit_id in applicant_status_reports
        $stmt1 = $conn->prepare("
            INSERT INTO applicant_status_reports (applicant_id, business_unit_id, from_status, to_status, report_text, admin_id)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        // types: i i s s s i
        $stmt1->bind_param("iisssi", $id, $currentBuId, $fromStatus, $to, $reportText, $adminId);
        $stmt1->execute();
        $stmt1->close();

        // Update applicants.status (BU-safe)
        $stmt2 = $conn->prepare("UPDATE applicants SET status=? WHERE id=? AND business_unit_id=?");
        $stmt2->bind_param("sii", $to, $id, $currentBuId);
        $stmt2->execute();
        $stmt2->close();
        ...
        $conn->rollback();
        error_log('SMC Status change failed: ' . $e->getMessage());
        $ok = false;
    }
    ...
    if ($ok && isset($auth) && method_exists($auth, 'logActivity') && isset($_SESSION['admin_id'])) {
        $label = $fullName ?: "ID {$id}";
        $auth->logActivity((int)$_SESSION['admin_id'], 'Update Applicant Status (with report)', "Updated status for {$label} → {$to}; Reason: " . mb_substr($reportText, 0, 200));
    }
...
/* =========================================================
 * GET handler – change status without report (BU-safe)
 * =========================================================*/
if (isset($_GET['action'], $_GET['id'], $_GET['to']) && $_GET['action'] === 'update_status') {
    ...
    if (in_array($to, $allowedStatuses, true)) {
        ...
        if ($conn instanceof mysqli) {
            $stmt = $conn->prepare("UPDATE applicants SET status=? WHERE id=? AND business_unit_id=?");
            if ($stmt) {
                $stmt->bind_param("sii", $to, $id, $currentBuId);
...
/* =========================================================
 * Load "on_process" applicants WITH latest booking per applicant (BU-scoped)
 * =========================================================*/
$applicants = [];
$conn = $database->getConnection();
if ($conn instanceof mysqli && $currentBuId > 0) {
    // Exclude active blacklists + soft-deleted
    $sql = "
        SELECT
            a.id,
            a.business_unit_id,
            a.first_name, a.middle_name, a.last_name, a.suffix,
            a.email, a.phone_number, a.preferred_location,
            a.picture, a.status, a.created_at,

            -- Latest booking fields (aliased for direct use in UI)
        ...
        LEFT JOIN (
            ...
            INNER JOIN (
              ...
              ON bm.applicant_id = b.applicant_id
             AND bm.business_unit_id = b.business_unit_id
             AND bm.max_created = b.created_at
        ) cb
          ON cb.applicant_id = a.id
         AND cb.business_unit_id = a.business_unit_id
        WHERE a.business_unit_id = ?
          AND a.status = 'on_process'
          AND a.deleted_at IS NULL
          AND NOT EXISTS (
              SELECT 1 FROM blacklisted_applicants ba
               WHERE ba.applicant_id = a.id AND ba.is_active = 1
          )
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="<?php echo h($exportReportsUrl); ?>" class="btn btn-outline-success">
                    ...
                    <?php else: ?>
                        <?php foreach ($applicants as $row): ?>
                            <?php
                                ...
                                $applicantName = h(getFullName($row['first_name'], $row['middle_name'], $row['last_name'], $row['suffix']));
                                ...
                                <td class="tbl-photo">
                                    <?php if (!empty($row['picture'])): ?>
                                        <img src="<?php echo h($photo); ?>"
                                             alt="Photo" class="rounded" width="50" height="50"
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="changeStatusBtn-<?php echo $id; ?>">
                                                <li>
                                                    <a class="dropdown-item <?php echo $currentStatus === 'pending' ? 'disabled' : ''; ?> change-status"
                                                       href="#"
                                                       data-href="<?php echo h($toPendingUrl); ?>"
                                                       data-id="<?php echo $id; ?>"
                                                       data-from="<?php echo h($currentStatus); ?>"
                                                       data-to="pending"
                                                ...
                                                <li>
                                                    <a class="dropdown-item <?php echo $currentStatus === 'on_process' ? 'disabled' : ''; ?> change-status"
                                                       href="#"
                                                       data-href="<?php echo h($toOnProcessUrl); ?>"
                                                       data-id="<?php echo $id; ?>"
                                                       data-from="<?php echo h($currentStatus); ?>"
                                                       data-to="on_process"
                                                       data-applicant="<?php echo $applicantName; ?>"
                                                ...
                                                <li>
                                                    <a class="dropdown-item <?php echo $currentStatus === 'approved' ? 'disabled' : ''; ?> change-status"
                                                       href="#"
                                                       data-href="<?php echo h($toApprovedUrl); ?>"
                                                       data-id="<?php echo $id; ?>"
                                                       data-from="<?php echo h($currentStatus); ?>"
                                                       data-to="approved"
                                                       data-applicant="<?php echo $applicantName; ?>"
...
<!-- === Status Change Report Modal (SMC) === -->
