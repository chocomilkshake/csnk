...
if ($canSeeCSNK) {
    ...
    $totalApplicants = csnk_count_bu($conn, "SELECT COUNT(*) FROM applicants WHERE business_unit_id=? AND deleted_at IS NULL{$notBlacklisted}", $buId);
    $pendingCount = csnk_count_bu($conn, "SELECT COUNT(*) FROM applicants WHERE business_unit_id=? AND status='pending' AND deleted_at IS NULL{$notBlacklisted}", $buId);
    $onProcessCount = csnk_count_bu($conn, "SELECT COUNT(*) FROM applicants WHERE business_unit_id=? AND status='on_process' AND deleted_at IS NULL{$notBlacklisted}", $buId);
...
if ($canViewReports && $conn instanceof mysqli) {
    $stmt = $conn->prepare("SELECT COUNT(*) FROM applicant_reports WHERE business_unit_id = ?");
    if ($stmt) {
        $stmt->bind_param('i', $buId);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    <aside class="sidebar" id="csnkSidebar">
        ...
        <nav class="sidebar-menu" aria-label="Primary">
            ...
            <?php if ($showRegionPlaceholders && $canSeeSMC): ?>
                ...
                <!-- SMC-Bahrain -->
                <button class="sidebar-item sidebar-toggle" type="button" data-bs-toggle="collapse"
                    data-bs-target="#smcBahrainMenu" aria-expanded="false" aria-controls="smcBahrainMenu"
