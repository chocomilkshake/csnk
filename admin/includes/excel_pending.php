<?php
/**
 * FILE: includes/excel_pending.php
 * PURPOSE: Export "Pending Applicants" to Excel (.xlsx) with optional search filter via ?q=
 * DESIGN: Matches your excel_deleted-applicants.php theme:
 *   - Logo top-right, centered title/subtitle
 *   - Zebra striping, thin borders, freeze panes, auto-filter
 *   - Excel date formatting (Date Applied)
 */
...
/* -------------------------------------------------
 *  Utilities (mirror pages/pending.php filtering)
 * -------------------------------------------------*/
...
/** Case-insensitive contains filter mirroring pages/pending.php */
function filterApplicantsByQuery(array $rows, string $query): array {
    ...
    return array_values(array_filter($rows, function(array $app) use ($needle) {
        ...
        $loc    = renderPreferredLocationXls($app['preferred_location'] ?? null); // full text
...
$sheet->setTitle('Pending Applicants');

$spreadsheet->getProperties()
    ->setCreator('CSNK Manpower Agency')
    ->setLastModifiedBy('CSNK Manpower Agency')
    ->setTitle('Pending Applicants Export')
    ->setSubject('Pending Applicants')
    ->setDescription('Export of pending applicants from CSNK Admin.')
    ->setCategory('Export');

// Defaults
$spreadsheet->getDefaultStyle()->getFont()->setName('Calibri')->setSize(11);

// Palette (consistent with your deleted exporter)
$ink          = 'FF111827'; // #111827
...
// Headers (exclude photo/action; mirror page list fields)
$headers = ['#', 'Name', 'Phone', 'Email', 'Location', 'Date Applied'];
...
// ------------- Download -------------
$filename = 'pending_applicants_' . date('Ymd_His') . '.xlsx';
