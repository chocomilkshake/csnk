 ...
 */
try {
  // NOTE: $database->prepare is used below assuming your Database class exposes mysqli via ->prepare
...
function renderWorkHistoryListHtml(?string $json): string
{
  ...
  foreach ($arr as $row) {
    ...
    $top = [];
    if ($company !== '')
      $top[] = '<span class="fw-semibold">' . safe($company) . '</span>';
    if ($role !== '')
      $top[] = safe($role);
    if (!empty($top))
      $line .= implode(' — ', $top);
...
/**
 * Helper: Format currency as PHP Peso with 2 decimals.
 */
    ...
    <div class="card">
      <div class="card-header bg-white py-2 d-flex align-items-center justify-content-between">
        ...
        <?php if ($latestBooking): ?>
          ...
          <span
            class="badge bg-<?php echo $bColor; ?> text-uppercase"><?php echo safe($latestBooking['status']); ?></span>
        <?php endif; ?>
      </div>
      <div class="card-body">
        <?php if (!$latestBooking): ?>
      ...
      <div class="modal-content">
        <div class="modal-header py-2">
          <h6 class="modal-title" id="applicantVideoModalLabel">
            <i
              class="bi bi-film me-2"></i><?php echo !empty($applicantData['video_title']) ? safe($applicantData['video_title']) : 'Applicant Video'; ?>
          </h6>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
