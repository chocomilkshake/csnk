<?php
// FILE: pages/turkey_view-turkey_approved.php
// Clone of view_turkey_approved.php but constrained for SMC (agency) + Turkey (country)

$pageTitle = 'Turkey — View Approved (Applicant + Client) — SMC';
...
// ---- LOCKS (adjust if needed) ----
const AGENCY_LOCK = 'SMC';     // <-- change to 'CSNK' if needed
const COUNTRY_LOCK = 'Turkey'; // Friendly country name
const COUNTRY_ISO2 = 'TR';     // ISO2

// Self page (for endpoints & redirects)
$PAGE_SELF = 'turkey_view-turkey_approved.php';
...
/* ------------------------------------------------------------------
   ...
   GET turkey_view-turkey_approved.php?action=history&id=123
-------------------------------------------------------------------*/
if (isset($_GET['action']) && $_GET['action'] === 'history' && isset($_GET['id'])) {
  ...
  try {
    ...
    error_log('History fetch (turkey_view-approved) failed: ' . $e->getMessage());
  }
...
/* ------------------------------------------------------------------
   ...
   POST turkey_view-turkey_approved.php  action=add_applicant_report
-------------------------------------------------------------------*/
if (
  ...
  if (!$validCsrf) {
    header('Location: ' . $PAGE_SELF . $qsBase . '&err=csrf');
    exit;
  }
  if ($noteText === '' || mb_strlen($noteText) < 3) {
    header('Location: ' . $PAGE_SELF . $qsBase . '&err=short');
    exit;
  }
  ...
  try {
    ...
    if ($ok) {
      ...
      header('Location: ' . $PAGE_SELF . $qsBase . '&saved=1'); // ✅ success flag
      exit;
    } else {
      header('Location: ' . $PAGE_SELF . $qsBase . '&err=save');
      exit;
    }
  } catch (Throwable $e) {
    error_log('Add report (turkey_view-approved) failed: ' . $e->getMessage());
    header('Location: ' . $PAGE_SELF . $qsBase . '&err=exception');
    exit;
  }
}

// After endpoints, start normal page output:
require_once '../includes/header.php';
require_once '../includes/Applicant.php';
$applicant = new Applicant($database);

// Re-apply q from GET if present (safety)
if (isset($_GET['q'])) {
  $q = trim((string) $_GET['q']);
...
if (!isset($_GET['id'])) {
  // If you have a Turkey-only list page, change this to 'turkey_turkey_approved.php'
  $dest = 'turkey_approved.php' . ($q !== '' ? ('?q=' . urlencode($q)) : '');
  redirect($dest);
  exit;
}
$id = (int) $_GET['id'];

/** Load Applicant (ensure not deleted) + optional agency/country guard if columns exist */
try {
  ...
  if (colExists($conn, 'applicants', 'country_code')) {
    $sql .= " AND country_code = ?";
    $types .= 's';
    $args[] = COUNTRY_ISO2;
  } elseif (colExists($conn, 'applicants', 'country')) {
    $sql .= " AND country = ?";
    $types .= 's';
    $args[] = COUNTRY_LOCK;
  } elseif (colExists($conn, 'applicants', 'country_name')) {
    $sql .= " AND country_name = ?";
    $types .= 's';
    $args[] = COUNTRY_LOCK;
  }

  // Agency filter if present in schema
  ...
  error_log('Applicant load (TR/SMC) failed: ' . $e->getMessage());
  $applicantData = false;
}

if (!$applicantData) {
  $dest = 'turkey_approved.php' . ($q !== '' ? ('?q=' . urlencode($q)) : '');
  header('Location: ' . $dest . '&err=notfound');
  exit;
}
...
try {
  ...
  $stmt2 = $conn->prepare("SELECT COUNT(*) AS c FROM applicant_reports WHERE applicant_id = ?");
...
$latestBooking = null;
try {
  // We only enforce applicant_id here (agency/country are implied by applicant),
  // but you can add extra guards the same way if your schema has these columns.
  $stmt = $conn->prepare("
    SELECT cb.*
...
function renderBadgesFromJson(?string $json, string $badgeClass = 'bg-light text-primary border', int $max = 0): string {
  ...
  if (is_array($arr)) {
    foreach ($arr as $v) {
      if (is_string($v) && trim($v) !== '') $items[] = trim($v);
    }
  } else {
    $fallback = trim($json, " \t\n\r\0\x0B[]\"");
    if ($fallback !== '') {
  ...
  foreach ($items as $label) {
    $out[] = '<span class="badge rounded-pill ' . $badgeClass . ' fw-semibold">' . safe($label) . '</span>';
  }
  return implode(' ', $out);
}
...
function renderServicesBadges(?string $json): string {
  ...
  foreach ($labels as $label) {
    $out[] = '<span class="badge rounded-pill bg-light text-dark border fw-semibold">' . safe($label) . '</span>';
  }
...
/** URLs (if you have a Turkey-only list page, set to 'turkey_turkey_approved.php') */
$backUrl   = 'turkey_approved.php' . ($q !== '' ? ('?q=' . urlencode($q)) : '');
$editUrl   = 'edit-applicant.php?id=' . $id . ($q !== '' ? ('&q=' . urlencode($q)) : '');
$printUrl  = 'print-applicant.php?id=' . $id . ($q !== '' ? ('&q=' . urlencode($q)) : '');
$historyUrl = 'view-applicant-history.php?id=' . (int) $id; // optional history page
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div class="d-flex align-items-center gap-3">
    <h4 class="mb-0 fw-semibold">Applicant + Client (Approved) — Turkey (<?= safe(AGENCY_LOCK) ?>)</h4>
      ...
      <div class="card-body">
        ...
        <div class="d-flex align-items-center gap-3 mb-3">
          <?php if (!empty($pictureUrl)): ?>
            <img src="<?php echo safe($pictureUrl); ?>" alt="Photo" class="rounded-circle"
        ...
        <?php else: ?>
          ...
          <div class="row g-3 mt-2">
            <div class="col-6">
              ...
              <div class="fw-semibold">
                <?php echo !empty($latestBooking['created_at']) ? safe(formatDateTime($latestBooking['created_at'])) : '—'; ?>
              </div>
            </div>
            <div class="col-6">
              <div class="small-label mb-1">Updated</div>
              <div class="fw-semibold">
                <?php echo !empty($latestBooking['updated_at']) ? safe(formatDateTime($latestBooking['updated_at'])) : '—'; ?>
              </div>
            </div>
          </div>
        <?php endif; ?>
...
<div class="card mt-3">
  ...
  <div class="card-body">
          ...
          <div class="list-group-item">
            <div class="d-flex justify-content-between align-items-center mb-1">
              ...
              <div class="text-muted small">
                <?php echo htmlspecialchars(formatDateTime((string)$rpt['created_at']), ENT_QUOTES, 'UTF-8'); ?>
              </div>
            </div>
            <div class="history-note">
              <?php echo nl2br(htmlspecialchars((string)($rpt['note_text'] ?? ''), ENT_QUOTES, 'UTF-8')); ?>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
    ...
    <!-- Add Report -->
    <form method="post"
      action="<?php echo $PAGE_SELF; ?>?id=<?php echo (int)$id . ($q !== '' ? ('&q=' . urlencode($q)) : ''); ?>">
      <div class="mb-2 text-muted small">Add a new report for <span class="fw-semibold"><?php echo $fullName; ?></span>
      </div>
      <div class="mb-3">
        <label for="note_text" class="form-label">Report / Notes <span class="text-danger">*</span></label>
        <textarea class="form-control" id="note_text" name="note_text" rows="4" required minlength="3" maxlength="4000"
  ...
  <div class="accordion-item">
    <h2 class="accordion-header" id="headingDocs">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseDocs"
        aria-expanded="false" aria-controls="collapseDocs">
        Applicant Documents
      </button>
    </h2>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btn) {
      btn.addEventListener('click', function () {
        ...
        fetch('<?php echo $PAGE_SELF; ?>?action=history&id=<?php echo (int) $id; ?>', { credentials: 'same-origin' })
