...
if (
  ...
  if (!$validCsrf) {
    ...
    header('Location: view_approved.php' . $qsBase . '&err=csrf');
    exit;
  }
  if ($noteText === '' || mb_strlen($noteText) < 3) {
    // too short: show inline error flag
    header('Location: view_approved.php' . $qsBase . '&err=short');
    exit;
  }
  ...
  try {
    ...
    if ($ok) {
      ...
      header('Location: view_approved.php' . $qsBase . '&err=save');
      exit;
    }
  } catch (Throwable $e) {
    error_log('Add report (view_approved) failed: ' . $e->getMessage());
    header('Location: view_approved.php' . $qsBase . '&err=exception');
    exit;
  }
}
...
function renderServicesBadges(?string $json): string
{
  ...
  if (json_last_error() !== JSON_ERROR_NONE)
    return '<span class="text-muted">' . safe($json) . '</span>';
  $labels = [];
  if (is_array($data)) {
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div class="d-flex align-items-center gap-3">
    ...
    <span
      class="badge bg-<?php echo $badgeColor; ?>"><?php echo safe(ucfirst(str_replace('_', ' ', $status))); ?></span>
  </div>
              ...
              foreach ($labels as $k => $label) {
                if (!empty($eduArr[$k]) && is_array($eduArr[$k])) {
                  ...
                  if (!empty($row['year']))
                    $parts[] = $row['year'];
                  if ($parts)
                    echo '<li class="small">' . safe($label) . ': ' . safe(implode(' • ', $parts)) . '</li>';
                }
              }
              echo '</ul>';
            } else {
            ...
            if (is_array($workArr) && $workArr) {
              ...
              foreach ($workArr as $w) {
                ...
                if (!empty($w['location']))
                  $parts[] = $w['location'];
                if ($parts)
                  echo '<li class="small">' . safe(implode(' — ', $parts)) . '</li>';
              }
              echo '</ul>';
            } else {
              echo '<div class="text-muted">N/A</div>';
...
<div class="card mt-3">
  ...
  <div class="card-body">
    ...
    <!-- Add Report -->
    <form method="post"
      action="view_approved.php?id=<?php echo (int) $id . ($q !== '' ? ('&q=' . urlencode($q)) : ''); ?>">
                          ...
                          <div class="row g-3">
                            <div class="col-md-6">
                              ...
                              <div class="d-flex align-items-center gap-2">
                                ...
                                <?php if ($emailB !== ''): ?>
                                  <a href="mailto:<?php echo safe($emailB); ?>"
                                    class="value-sm text-decoration-none"><?php echo safe($emailB); ?></a>
                                <?php else: ?>
                            ...
                            <div class="col-md-6">
                              ...
                              <div class="d-flex align-items-center gap-2">
                                ...
                                <?php if ($phoneB !== ''): ?>
                                  <a href="tel:<?php echo safe($phoneB); ?>"
                                    class="value-sm text-decoration-none"><?php echo safe($phoneB); ?></a>
                                <?php else: ?>
                                  <span class="text-muted">N/A</span>
                                <?php endif; ?>
                              </div>
