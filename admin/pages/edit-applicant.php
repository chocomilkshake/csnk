...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $knownMap = [];
    foreach ($known as $k)
        $knownMap[norm_label($k)] = $k;
    $specializations = [];
    foreach ($specializationsRaw as $sel) {
        $key = norm_label($sel);
        if ($key !== '' && isset($knownMap[$key]))
            $specializations[] = $knownMap[$key];
    }
    ...
    $_POST['daily_rate'] = isset($applicantData['daily_rate']) && $applicantData['daily_rate'] !== null
        ? number_format((float) $applicantData['daily_rate'], 2, '.', '')
        : '';
...
<form method="POST" enctype="multipart/form-data" id="editApplicantForm">
                ...
                <div class="col-md-6">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">First Name <span class="text-danger">*</span>
                                <input type="text" class="form-control" name="first_name" autocomplete="given-name"
                                    required
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Last Name <span class="text-danger">*</span>
                                <input type="text" class="form-control" name="last_name" autocomplete="family-name"
                                    required
                    ...
                    <div class="col-md-12">
                        <label class="form-label">Address <span class="text-danger">*</span>
                            <textarea class="form-control" name="address" autocomplete="address" rows="2" required
                                style="width:600px"><?= htmlspecialchars($_POST['address'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                        </label>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <label class="form-label">Highest Educational Level <span class="text-danger">*</span>
                        <select class="form-select" name="education_level" required>
                            <option value="">Select...</option>
                            <?php $opts = getEducationLevelOptions();
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <label class="form-label">Year
                        <input type="text" class="form-control" name="edu[elementary][year]"
                ...
                <div class="col-md-4">
                    <label class="form-label">Year
                        <input type="text" class="form-control" name="edu[highschool][year]"
                ...
                <div class="col-md-3">
                    <label class="form-label">Year
                        <input type="text" class="form-control" name="edu[senior_high][year]"
                ...
                <div class="col-md-3">
                    <label class="form-label">Year
                        <input type="text" class="form-control" name="edu[college][year]"
                ...
                foreach ($opts as $i => $opt):
                    ...
                    <div class="col-md-6 mb-2">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="skill<?= (int) $i ?>"
                                name="specialization_skills[]" value="<?= htmlspecialchars($opt, ENT_QUOTES, 'UTF-8') ?>"
                                <?= $isChecked ?>>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <p class="text-muted">Upload a file to replace/append a document. New uploads will be added as records for
                this applicant.</p>
            <div class="row g-3">
                <?php
                $docTypes = [
                    ...
                    <div class="col-md-6">
                        <label class="form-label"><?= htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?>
                            <input type="file" class="form-control"
                                name="<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>" accept=".pdf,.jpg,.jpeg,.png">
                        </label>
                    </div>
                <?php endforeach; ?>
            </div>
