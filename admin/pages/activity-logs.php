...
if ($conn instanceof mysqli) {
  ...
  if ($rs = $conn->query($q))
    $adminUsers = $rs->fetch_all(MYSQLI_ASSOC);
}
...
if ($conn instanceof mysqli) {
  ...
  if ($isAdminOnly)
    $q .= " WHERE au.role <> 'super_admin'";
  $q .= " ORDER BY al.created_at DESC LIMIT 250";
  if ($rs = $conn->query($q))
    $logs = $rs->fetch_all(MYSQLI_ASSOC);
}
...
function enrichDescription(string $desc, array $appMap, array $userMap): string
{
  $desc = preg_replace_callback(
    '/\b(Applicant|applicant)([^0-9]{0,30}ID[:\s#]*)([0-9]+)\b/',
    fn($m) => isset($appMap[(int) $m[3]]) ? ($m[1] . $m[2] . $m[3] . ' — ' . $appMap[(int) $m[3]]) : $m[0],
    $desc
  );
  $desc = preg_replace_callback(
    '/\b(Admin|User|admin|user)([^0-9]{0,30}ID[:\s#]*)([0-9]+)\b/',
    fn($m) => isset($userMap[(int) $m[3]]) ? ($m[1] . $m[2] . $m[3] . ' — ' . $userMap[(int) $m[3]]) : $m[0],
    $desc
  );
  return $desc;
}
      ...
      <tbody>
        ...
        <?php else:
          foreach ($logs as $log):
            ...
            ?>
            <tr class="hover:bg-slate-50 cursor-pointer" data-user-id="<?= $log['admin_id'] ?>"
              ...
              data-avatar="<?= htmlspecialchars($avatarUrl, ENT_QUOTES, 'UTF-8') ?>" data-log-id="<?= $log['id'] ?>"
                  ...
                  <div class="position-relative" style="width:40px;height:40px;">
                    <?php if ($avatarUrl): ?>
                      <img src="<?= htmlspecialchars($avatarUrl, ENT_QUOTES, 'UTF-8') ?>"
                        alt="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?>" class="rounded-circle object-fit-cover"
                        width="40" height="40" loading="lazy">
                    <?php else: ?>
                      <div
                        class="rounded-circle bg-gradient d-flex align-items-center justify-content-center text-white fw-bold"
              ...
              <td><span
                  class="badge bg-light text-dark text-capitalize"><?= htmlspecialchars(str_replace('_', ' ', $log['role'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></span>
              </td>
              <td><span
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body pt-0">
        ...
        <div class="d-flex gap-3 align-items-start mb-3">
          <div class="rounded-circle overflow-hidden" style="width:56px;height:56px;">
            ...
            <div id="mAvatarIni" class="w-100 h-100 d-flex align-items-center justify-content-center text-white fw-bold"
      ...
      <div class="modal-footer border-0">
        <button id="mCopy" type="button" class="btn btn-outline-secondary"><i class="bi bi-clipboard me-1"></i>Copy
          Summary</button>
...
<script>
  ...
  (function () {
    ...
    const inRange = (d, range) => {
      if (!d || range === 'all') return true; const now = new Date(), day = 86400000, diff = now - d;
  ...
  (function () {
    ...
    document.querySelectorAll('#logTable tbody tr').forEach(tr => {
      tr.addEventListener('click', () => {
        ...
        mCopy.onclick = () => {
          if (!copyText.trim()) return; navigator.clipboard.writeText(copyText).then(() => {
