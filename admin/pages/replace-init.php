...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    if ($isAjax)
        json_out(false, ['message' => 'Invalid method.'], 405);
...
if ($originalId <= 0 || $adminId === null) {
    if ($isAjax)
        json_out(false, ['message' => 'Missing required fields.'], 422);
...
if (!empty($_POST['csrf_token'])) {
    if (empty($_SESSION['csrf_token']) || !hash_equals((string) $_SESSION['csrf_token'], (string) $_POST['csrf_token'])) {
        if ($isAjax)
            json_out(false, ['message' => 'Invalid security token.'], 403);
...
if (!$replacementId) {
    if ($isAjax)
        json_out(false, ['message' => 'Failed to start replacement.'], 500);
