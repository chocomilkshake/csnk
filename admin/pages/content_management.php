                          ...
                          <td>
                            <div class="btn-group btn-group-sm">
                              ...
                              <form method="POST" class="d-inline">
                                ...
                                <input type="hidden" name="business_unit_id" value="<?= (int) $activeBUId ?>">
                                <button type="submit"
                                  class="btn btn-outline-<?= !empty($cat['is_active']) ? 'warning' : 'success' ?>"
                                  title="<?= !empty($cat['is_active']) ? 'Deactivate' : 'Activate' ?>">
                                  <i class="bi bi-<?= !empty($cat['is_active']) ? 'eye-slash' : 'eye' ?>"></i>
                                </button>
                              </form>
                              <form method="POST" class="d-inline"
                                onsubmit="return confirm('Are you sure? This will also delete all images in this category.');">
            ...
            <div class="card-body">
              <form id="bulkForm" method="POST" enctype="multipart/form-data" onsubmit="return false;">
                ...
                <div class="mb-3">
                  ...
                  <textarea class="form-control" name="content_description" id="bulkDesc" rows="2"
                    placeholder="Optional description for all uploads"></textarea>
                </div>

                <!-- Dropzone -->
                <div id="dropzone" class="border-2 border-dashed rounded-3 p-4 text-center bg-light position-relative"
                ...
                <div class="d-flex gap-2 mt-3">
                  <button id="btnClearAll" type="button" class="btn btn-outline-secondary btn-sm" disabled>Clear
                    All</button>
                        ...
                        <tr>
                          <td>
                            <?php if (!empty($item['image_path'])): ?>
                              <img src="<?= getFileUrl($item['image_path']) ?>" alt="" class="rounded"
                          ...
                          <td>
                            <div class="btn-group btn-group-sm">
                              <button class="btn btn-outline-primary" data-bs-toggle="modal"
                                data-bs-target="#editContentModal<?= $item['id'] ?>" title="Edit">
                                <i class="bi bi-pencil"></i>
                              </button>
                              <form method="POST" class="d-inline">
                                ...
                                <input type="hidden" name="business_unit_id" value="<?= (int) $activeBUId ?>">
                                <button type="submit"
                                  class="btn btn-outline-<?= !empty($item['is_active']) ? 'warning' : 'success' ?>"
                                  title="<?= !empty($item['is_active']) ? 'Hide' : 'Show' ?>">
                            ...
                            <div class="modal-content">
                              ...
                              <form method="POST" enctype="multipart/form-data">
                                <div class="modal-body">
                                  ...
                                  <div class="mb-3">
                                    <label class="form-label">Description</label>
                                    <textarea class="form-control" name="content_description"
                                      rows="2"><?= htmlspecialchars($item['description'] ?? '') ?></textarea>
                                  </div>
                                  <div class="mb-3">
                                    ...
                                    <?php if (!empty($item['image_path'])): ?>
                                      <div class="mt-2">
                                        <img src="<?= getFileUrl($item['image_path']) ?>" class="img-thumbnail"
