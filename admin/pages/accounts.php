...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_account'])) {
  ...
  if ($email !== '') {
    ...
    if (!$ok)
      $errors[] = $err . ($suggest ? ' Suggested: ' . htmlspecialchars($suggest, ENT_QUOTES, 'UTF-8') : '');
  }
  if ($password !== '') {
      ...
      <table class="table table-hover align-middle">
        ...
        <tbody>
          <?php if (empty($employeeAccounts)): ?>
            <tr>
              <td colspan="7" class="text-center text-muted">No employee accounts.</td>
            </tr>
          <?php else:
                  ...
                  <?php if ($isSuperAdmin): ?>
                    <div class="btn-group">
                      <button type="button" class="btn btn-sm btn-secondary" data-bs-toggle="modal"
                        data-bs-target="#resetPasswordModal" data-user-id="<?php echo (int) $acc['id']; ?>"
                      ...
                      <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal"
                        data-bs-target="#editAccountModal" data-user-id="<?php echo (int) $acc['id']; ?>"
                        ...
                        data-agency="<?php echo htmlspecialchars((string) $acc['agency']); ?>" data-role="employee"
                        title="Edit"><i class="bi bi-pencil-square"></i></button>

                      <?php if ((int) $acc['id'] !== (int) $_SESSION['admin_id']): ?>
                        <a href="accounts.php?action=delete&id=<?php echo (int) $acc['id']; ?>" class="btn btn-sm btn-danger"
                          onclick="return confirm('Delete this account?');" title="Delete"><i class="bi bi-trash"></i></a>
                      <?php endif; ?>
                    </div>
                  <?php elseif ($isAdmin): ?>
                    <div class="btn-group">
        ...
        <table class="table table-hover align-middle">
          ...
          <tbody>
            <?php if (empty($adminAccounts)): ?>
              <tr>
                <td colspan="6" class="text-center text-muted">No admin accounts.</td>
              </tr>
            <?php else:
                    ...
                    <?php if ($isSuperAdmin): ?>
                      <div class="btn-group">
                        ...
                        <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal"
                          ...
                          data-fullname="<?php echo htmlspecialchars($acc['full_name']); ?>"
                          data-email="<?php echo htmlspecialchars($acc['email']); ?>"
                          data-status="<?php echo htmlspecialchars($acc['status']); ?>" data-agency="" data-role="admin"
                          title="Edit"><i class="bi bi-pencil-square"></i></button>
        ...
        <table class="table table-hover align-middle">
          ...
          <tbody>
            <?php if (empty($superAccounts)): ?>
              <tr>
                <td colspan="6" class="text-center text-muted">No super admin accounts.</td>
              </tr>
            <?php else:
              foreach ($superAccounts as $acc): ?>
                <tr>
                  <td>
                    <strong><?php echo htmlspecialchars($acc['username']); ?></strong>
                    <?php if ((int) $acc['id'] === (int) $_SESSION['admin_id']): ?>
                  ...
                  <td>
                    <div class="btn-group">
                      ...
                      <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal"
                        ...
                        data-fullname="<?php echo htmlspecialchars($acc['full_name']); ?>"
                        data-email="<?php echo htmlspecialchars($acc['email']); ?>"
                        data-status="<?php echo htmlspecialchars($acc['status']); ?>" data-agency="" data-role="super_admin"
                        title="Edit"><i class="bi bi-pencil-square"></i></button>
