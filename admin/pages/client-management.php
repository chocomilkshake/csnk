...
<div class="p-8 max-w-7xl mx-auto">
  ...
  <!-- Filter Tabs -->
  <div class="flex bg-gray-100 rounded-xl p-2 mb-8 gap-2">
    <a href="client-management.php"
      class="flex-1 px-6 py-3 rounded-lg font-semibold text-sm <?= $applicantStatus === 'all' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-700 hover:bg-white hover:shadow-sm hover:text-gray-900'; ?>">
      All <span
        class="ml-2 px-2 py-1 bg-white/50 rounded-lg text-xs font-bold <?= $applicantStatus === 'all' ? 'bg-white text-blue-900' : 'text-gray-600'; ?>"><?= $statusCounts['total'] ?></span>
    </a>
    <a href="?status=on_process"
      class="flex-1 px-6 py-3 rounded-lg font-semibold text-sm <?= $applicantStatus === 'on_process' ? 'bg-cyan-500 text-white shadow-md' : 'text-gray-700 hover:bg-white hover:shadow-sm hover:text-gray-900'; ?>">
      On Process <span
        class="ml-2 px-2 py-1 bg-white/50 rounded-lg text-xs font-bold <?= $applicantStatus === 'on_process' ? 'bg-white text-cyan-900' : 'text-gray-600'; ?>"><?= $statusCounts['on_process'] ?></span>
    </a>
    <a href="?status=approved"
      class="flex-1 px-6 py-3 rounded-lg font-semibold text-sm <?= $applicantStatus === 'approved' ? 'bg-emerald-500 text-white shadow-md' : 'text-gray-700 hover:bg-white hover:shadow-sm hover:text-gray-900'; ?>">
      Approved <span
        class="ml-2 px-2 py-1 bg-white/50 rounded-lg text-xs font-bold <?= $applicantStatus === 'approved' ? 'bg-white text-emerald-900' : 'text-gray-600'; ?>"><?= $statusCounts['approved'] ?></span>
    </a>
  </div>
