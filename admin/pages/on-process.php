...
if (
    ...
    if ($reportText === '' || mb_strlen($reportText) < 5) {
        if (function_exists('setFlashMessage'))
            setFlashMessage('error', 'Please provide a brief reason (at least 5 characters).');
        redirect('on-process.php' . $qs);
        exit;
    }
    ...
    if (!isset($database) || !method_exists($database, 'getConnection')) {
        if (function_exists('setFlashMessage'))
            setFlashMessage('error', 'Database connection unavailable.');
        redirect('on-process.php' . $qs);
        exit;
    }
    ...
    if (function_exists('setFlashMessage')) {
        if ($ok)
            setFlashMessage('success', 'Status updated and report saved.');
        else
            setFlashMessage('error', 'Failed to update status or save report. Please try again.');
    }
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="<?php echo htmlspecialchars($exportReportsUrl, ENT_QUOTES, 'UTF-8'); ?>"
            class="btn btn-outline-success">
            <i class="bi bi-journal-text me-2"></i>Export Reports
        </a>
        <a href="<?php echo htmlspecialchars($printReportsUrl, ENT_QUOTES, 'UTF-8'); ?>"
...
<div class="mb-3 d-flex justify-content-end">
    <form method="get" action="on-process.php" class="w-100" style="max-width: 420px;">
        <div class="input-group">
            <input type="text" name="q" class="form-control" placeholder="Search on-process (applicant or client)..."
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end"
                                            ...
                                            <li>
                                                <a class="dropdown-item <?php echo $currentStatus === 'pending' ? 'disabled' : ''; ?> change-status"
                                                    ...
                                                    data-to="pending" data-applicant="<?php echo $applicantName; ?>">
                                            ...
                                            <li>
                                                <a class="dropdown-item <?php echo $currentStatus === 'on_process' ? 'disabled' : ''; ?> change-status"
                                                    ...
                                                    data-id="<?php echo $id; ?>"
                                                    data-from="<?php echo htmlspecialchars($currentStatus, ENT_QUOTES, 'UTF-8'); ?>"
                                                    data-to="on_process" data-applicant="<?php echo $applicantName; ?>">
                                            ...
                                            <li>
                                                <a class="dropdown-item <?php echo $currentStatus === 'approved' ? 'disabled' : ''; ?> change-status"
                                                    ...
                                                    data-id="<?php echo $id; ?>"
                                                    data-from="<?php echo htmlspecialchars($currentStatus, ENT_QUOTES, 'UTF-8'); ?>"
                                                    data-to="approved" data-applicant="<?php echo $applicantName; ?>">
...
<div class="modal fade" id="statusReportModal" tabindex="-1" aria-labelledby="statusReportModalLabel"
    aria-hidden="true">
        ...
        <form method="post" action="on-process.php" id="statusReportForm" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="sr-text" name="report_text" rows="4" required minlength="5"
                        maxlength="2000" placeholder="Write a brief reason for this status change..."></textarea>
