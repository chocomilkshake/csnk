...
function renderServicesBadges(?string $json, int $max = 0): string
{
  ...
  if (json_last_error() !== JSON_ERROR_NONE)
    return '<span class="text-muted">' . safe(truncate($json, 60)) . '</span>';
  $labels = [];
  if (is_array($data)) {
...
<head>
  ...
  <style>
    :root {
      ...
      /* matches @page margin */
      --doc-head: 6mm;
      /* header band on each document page */
      --doc-gap: 2mm;
    ...
    .doc-embed {
      ...
      object-fit: contain;
      /* change to 'cover' if you prefer edge-to-edge even if cropped */
      border: 0;
    }
  </style>
</head>
      ...
      <div class="col-6">
        <div class="border rounded p-2">
          <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span
              class="badge <?php echo $statusClass; ?> rounded-pill"><?php echo safe(ucfirst(str_replace('_', ' ', $status))); ?></span>
          </div>

          <div class="d-flex align-items-center gap-2 mb-2">
            <?php if (!empty($pictureUrl)): ?>
              <img src="<?php echo safe($pictureUrl); ?>" class="rounded-circle"
          ...
          <div class="row g-2 small">
            <div class="col-6">
              ...
              <div class="fw-semibold text-truncate"><?php echo $primaryPhone !== '' ? safe($primaryPhone) : 'N/A'; ?>
              </div>
            </div>
            <div class="col-6">
                ...
                foreach ($labels as $k => $label) {
                  if (!empty($eduArr[$k]) && is_array($eduArr[$k])) {
                    ...
                    if (!empty($row['year']))
                      $parts[] = $row['year'];
                    if ($parts)
                      echo '<li class="small">' . safe($label) . ': ' . safe(truncate(implode(' • ', $parts), 88)) . '</li>';
                  }
                }
                echo '</ul>';
              } else {
              ...
              if (is_array($workArr) && $workArr) {
                ...
                foreach ($workArr as $w) {
                  ...
                  if (!empty($w['location']))
                    $parts[] = $w['location'];
                  if ($parts)
                    echo '<li class="small">' . safe(truncate(implode(' — ', $parts), 88)) . '</li>';
                }
                echo '</ul>';
              } else {
                echo '<div class="text-muted">N/A</div>';
      ...
      <div class="col-6">
        <div class="border rounded p-2">
          <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="badge rounded-pill bg-secondary">Total Client Bookings:
              <?php echo (int) $totalBookings; ?></span>
          </div>
            ...
            <div class="row g-2 small">
              ...
              <div class="col-6">
                ...
                <div class="fw-semibold">
                  <?php echo !empty($latestBooking['created_at']) ? safe(formatDate($latestBooking['created_at'])) : '—'; ?>
                </div>
              </div>
              <div class="col-6">
                <div class="text-muted">Updated</div>
                <div class="fw-semibold">
                  <?php echo !empty($latestBooking['updated_at']) ? safe(formatDate($latestBooking['updated_at'])) : '—'; ?>
                </div>
              </div>
            </div>
          <?php endif; ?>
