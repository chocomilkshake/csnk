<?php
// FILE: pages/csnk_view-onprocess.php
// Clone of view-onprocess.php — constrained for CSNK agency (auto-detects schema columns)

$pageTitle = 'On Process — CSNK';
...
// Optional mysqli connection if your Database class exposes it (used for safe column checks)
$conn = null;
if (isset($database) && is_object($database) && method_exists($database, 'getConnection')) {
  $conn = $database->getConnection(); // mysqli
}

/**
 * Preserve list search (if user came from turkey_on-process.php with ?q=)
 */
$q = '';
if (isset($_GET['q'])) {
...
if (!isset($_GET['id'])) {
  $dest = 'turkey_on-process.php' . ($q !== '' ? ('?q=' . urlencode($q)) : '');
  redirect($dest);
  exit;
}
...
/**
 * Check if a column exists (cached) — safe even if no $conn available.
 */
function colExists(?mysqli $conn, string $table, string $col): bool
{
  ...
  if (!($conn instanceof mysqli)) return $cache[$key] = false;
  try {
...
/**
 * Load Applicant (ensure not deleted) + constrain to CSNK if schema has an agency column.
 */
try {
  // Base
  $sql = "SELECT * FROM applicants WHERE id = ? AND (status <> 'deleted' OR status IS NULL)";
  $types = 'i';
  $args = [$id];

  // Constrain to CSNK if your schema has an agency column
  // It will try these in order and only append conditions for columns that exist:
  //   agency_code / agency / agency_name
  $AGENCY = 'CSNK';
  if (colExists($conn, 'applicants', 'agency_code')) {
    $sql .= " AND agency_code = ?";
    $types .= 's';
    $args[] = $AGENCY;
  } elseif (colExists($conn, 'applicants', 'agency')) {
    $sql .= " AND agency = ?";
    $types .= 's';
    $args[] = $AGENCY;
  } elseif (colExists($conn, 'applicants', 'agency_name')) {
    $sql .= " AND agency_name = ?";
    $types .= 's';
    $args[] = $AGENCY;
  }
  ...
  // NOTE: $database->prepare is used assuming your Database class exposes mysqli via ->prepare
...
if (!$applicantData) {
  setFlashMessage('error', 'Applicant not found or not under CSNK.');
  $dest = 'turkey_on-process.php' . ($q !== '' ? ('?q=' . urlencode($q)) : '');
  redirect($dest);
  exit;
}
    ...
    $out[] = '
      <div class="mb-2">
        <div class="d-flex align-items-center gap-2">
          ...
          ' . ($year !== '' ? '<span class="badge bg-light text-dark border">' . safe($year) . '</span>' : '') . '
        </div>
...
function renderWorkHistoryListHtml(?string $json): string
{
  ...
  foreach ($arr as $row) {
    ...
    if ($company === '' && $role === '' && $duration === '' && $location === '') continue;
    $line = '<li class="mb-1">';
    $top = [];
    ...
    $meta = [];
    if ($duration !== '')   $meta[] = safe($duration);
  ...
  if (is_array($data)) {
    ...
    if ($labels) {
      ...
      foreach ($labels as $lbl)
        $chips[] = '<span class="badge bg-light text-dark border fw-semibold">' . safe($lbl) . '</span>';
...
$dailyRateDisplay = formatCurrency($applicantData['daily_rate'] ?? null);

$backUrl = 'turkey_on-process.php' . ($q !== '' ? ('?q=' . urlencode($q)) : '');
// Keep your edit/print/history routes the same:
$editUrl = 'edit-applicant.php?id=' . $id . ($q !== '' ? ('&q=' . urlencode($q)) : '');
$printUrl = 'print-applicant.php?id=' . (int) $id . ($q !== '' ? '&q=' . urlencode($q) : '');
$historyUrl = 'view-applicant-history.php?id=' . (int) $id; // History button target

// Role flags available from header.php: $isAdmin, $isSuperAdmin
?>
<style>
  /* ===== Modern page header actions ===== */
  .page-actions .btn { white-space: nowrap; }

  /* Small labels for info blocks */
  .small-label {
    font-size: .8rem;
    color: #6c757d;
...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
  <div class="d-flex align-items-center gap-3">
    <h4 class="mb-0 fw-semibold">Applicant & Clients — <span class="text-primary">CSNK</span></h4>
...
<!-- MAIN ROW: Applicant (left) + Latest Booking (right) -->
<div class="row g-3">
  <!-- Applicant Panel -->
  <div class="col-xl-6">
    <div class="card">
      ...
      <div class="card-body">
        <div class="d-flex align-items-center gap-3 mb-3">
          <?php if (!empty($pictureUrl)): ?>
            <img src="<?php echo safe($pictureUrl); ?>" alt="Profile" class="rounded-circle border"
          ...
          <div class="flex-grow-1">
            ...
            <small class="text-muted d-block mt-1">Applied:
              <?php echo formatDate($applicantData['created_at']); ?></small>
          </div>
        </div>
        ...
        <div class="row g-3">
          ...
          <div class="col-6">
            ...
            <div class="fw-semibold">
              <?php $yrs = (int) ($applicantData['years_experience'] ?? 0);
          ...
          <div class="col-6">
            ...
            <div class="fw-semibold">
              <?php echo !empty($applicantData['employment_type']) ? safe($applicantData['employment_type']) : 'N/A'; ?>
            </div>
          </div>
          <div class="col-6">
            <div class="small-label mb-1">Daily Rate</div>
            <div class="fw-semibold">
              <?php echo htmlspecialchars($dailyRateDisplay, ENT_QUOTES, 'UTF-8'); ?>
            </div>
          </div>
          <div class="col-12">
        ...
        <?php else: ?>
          ...
          <div class="row g-3 mt-2">
            <div class="col-6">
              ...
              <div class="fw-semibold">
                <?php echo !empty($latestBooking['created_at']) ? formatDateTime($latestBooking['created_at']) : '—'; ?>
              </div>
            </div>
            <div class="col-6">
              <div class="small-label mb-1">Updated</div>
              <div class="fw-semibold">
                <?php echo !empty($latestBooking['updated_at']) ? formatDateTime($latestBooking['updated_at']) : '—'; ?>
              </div>
            </div>
          </div>
        <?php endif; ?>
...
<div class="accordion mt-3" id="extraInfoAccordion">
  ...
  <div class="accordion-item">
    <h2 class="accordion-header" id="headingOne">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
              data-bs-target="#collapseApplicantDetails" aria-expanded="false" aria-controls="collapseApplicantDetails">
        More Applicant Details (Education & Work)
      </button>
    </h2>
    <div id="collapseApplicantDetails" class="accordion-collapse collapse" aria-labelledby="headingOne"
      ...
      <div class="accordion-body">
        <div class="row g-3">
          <div class="col-lg-6">
            <div class="card">
              <div class="card-header bg-white py-2">
                <h6 class="mb-0 fw-semibold"><i class="bi bi-mortarboard me-2"></i>Educational Attainment</h6>
              </div>
          ...
          <div class="col-lg-6">
            <div class="card">
              <div class="card-header bg-white py-2">
                <h6 class="mb-0 fw-semibold"><i class="bi bi-briefcase me-2"></i>Work History</h6>
              </div>
  ...
  <div class="accordion-item">
    <h2 class="accordion-header" id="headingTwo">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
              data-bs-target="#collapseAllBookings" aria-expanded="false" aria-controls="collapseAllBookings">
        Applicant's Client Engagement Record
      </button>
    </h2>
    <div id="collapseAllBookings" class="accordion-collapse collapse" aria-labelledby="headingTwo"
         data-bs-parent="#extraInfoAccordion">
      <div class="accordion-body">
              ...
              <tbody>
                <?php foreach ($allBookings as $i => $b): ?>
                  <?php
                    ...
                    $mailto = $email !== '' ? ('mailto:' . rawurlencode($email) . '?subject=' . $subject . '&body=' . $body) : '';
                  ...
                  <tr>
                    ...
                    <td>
                      <?php echo (!empty($b['appointment_date']) ? formatDate($b['appointment_date']) : '—') . ' ' . (!empty($b['appointment_time']) ? safe($b['appointment_time']) : ''); ?>
                    </td>
                    ...
                    <td class="text-center">
                      <div class="btn-group">
                        ...
                        <?php endif; ?>
                        <button type="button" class="btn btn-sm btn-outline-success" title="Show Contact"
                                data-bs-toggle="modal" data-bs-target="#<?php echo safe($modalId); ?>">
                          <i class="bi bi-telephone"></i>
                        </button>
                      </div>
                    </td>
                    <td><?php echo !empty($b['created_at']) ? formatDateTime($b['created_at']) : '—'; ?></td>
                  </tr>

                  <!-- Contact Modal -->
                  <div class="modal fade" id="<?php echo safe($modalId); ?>" tabindex="-1"
                       aria-labelledby="<?php echo safe($modalId); ?>Label" aria-hidden="true">
                          ...
                          <div class="row g-3">
                            <div class="col-md-6">
                              ...
                              <div class="d-flex align-items-center gap-2">
                                ...
                                <?php if ($email !== ''): ?>
                                  <a href="mailto:<?php echo safe($email); ?>"
                                     class="fw-semibold text-decoration-none"><?php echo safe($email); ?></a>
                                <?php else: ?>
                            ...
                            <div class="col-md-6">
                              ...
                              <div class="d-flex align-items-center gap-2">
                                ...
                                <?php if ($phone !== ''): ?>
                                  <a href="tel:<?php echo safe($phone); ?>"
                                     class="fw-semibold text-decoration-none"><?php echo safe($phone); ?></a>
                                <?php else: ?>
                                  <span class="text-muted">N/A</span>
                                <?php endif; ?>
                              </div>
  ...
  <div class="accordion-item">
    ...
    <div id="collapseDocuments" class="accordion-collapse collapse" aria-labelledby="headingThree"
         data-bs-parent="#extraInfoAccordion">
      <div class="accordion-body">
...
<!-- Video Modal -->
<?php if (!empty($applicantData['video_url'])): ?>
  <div class="modal fade" id="applicantVideoModal" tabindex="-1" aria-labelledby="applicantVideoModalLabel"
       aria-hidden="true">
