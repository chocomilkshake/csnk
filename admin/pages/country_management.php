...
if ($conn instanceof mysqli) {
  ...
  if ($fix = $conn->query("SELECT COUNT(*) AS cnt FROM countries WHERE id = 0")) {
    ...
    if ($cnt > 0) {
      ...
      $nr && $nr->close();
      while ($cnt--) {
        @$conn->query("UPDATE countries SET id={$nxt} WHERE id=0 LIMIT 1");
        $nxt++;
      }
    }
  }
...
if (isset($_GET['action'], $_GET['id'])) {
  ...
  if ($_GET['action'] === 'delete') {
    ...
    if ($buCount > 0) {
      ...
      $stmt->bind_param('i', $targetId);
      if ($stmt->execute())
        setFlashMessage('success', 'Country deleted.');
      else
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_country'])) {
  ...
  // Validation
  if (strlen($iso2) !== 2)
    $errors[] = "ISO2 must be exactly 2 characters.";
  if (strlen($iso3) !== 3)
...
<div class="row g-2 align-items-center toolbar mb-3">
  <div class="col-sm-6">
    <div class="input-icon">
      ...
      <input id="tblFilter" type="search" class="form-control"
        placeholder="Search by name or ISO2 (e.g., Philippines or PH)" aria-label="Search countries">
    </div>
  </div>
...
<div class="card shadow-sm">
  ...
  <div class="card-body p-0 table-wrap">
    <table class="table table-hover align-middle mb-0 border border-gray-300" id="countriesTable">
      <thead>
        <tr>
          <th class="text-center border border-gray-300">ID</th>
          <th class="text-center border border-gray-300">Name</th>
          <th class="text-center border border-gray-300">Codes</th>
        ...
        <?php else:
          foreach ($countries as $c): ?>
            ...
            <tr data-name="<?php echo strtolower(h($c['name'])); ?>" data-iso2="<?php echo strtolower(h($c['iso2'])); ?>"
              ...
              <td class="mono text-muted small text-center border border-gray-300"><?php echo (int) $c['id']; ?></td>
              <td class="ps-3 text-nowrap border border-gray-300">
                <?php echo $flag; ?>
                <strong><?php echo h($c['name']); ?></strong>
              </td>
              <td class="text-nowrap text-center border border-gray-300">
                <span class="badge badge-soft badge-iso2 badge-copy me-1" data-copy="<?php echo h($c['iso2']); ?>"
                  data-bs-toggle="tooltip" title="Click to copy ISO2">
                  <?php echo h($c['iso2']); ?>
                </span>
                <span class="badge badge-soft badge-iso3 badge-copy" data-copy="<?php echo h($c['iso3']); ?>"
                  data-bs-toggle="tooltip" title="Click to copy ISO3">
                  <?php echo h($c['iso3']); ?>
                </span>
              </td>
              <td class="text-nowrap text-center border border-gray-300">
                <span class="badge badge-soft badge-ph badge-copy" data-copy="<?php echo h($c['phone_country_code']); ?>"
                  data-bs-toggle="tooltip" title="Click to copy Phone Code">
                  <?php echo h($c['phone_country_code']); ?>
                </span>
              </td>
              <td class="mono text-nowrap text-center border border-gray-300">
                <span class="badge badge-soft badge-cur badge-copy" data-copy="<?php echo h($c['currency_code']); ?>"
                  data-bs-toggle="tooltip" title="Click to copy Currency Code">
              ...
              <td class="text-center border border-gray-300">
                <div class="btn-group">
                  ...
                  <button type="button" class="btn btn-info btn-sm btn-action text-white" data-bs-toggle="modal"
                    data-bs-target="#viewModal" data-id="<?php echo (int) $c['id']; ?>"
                    data-name="<?php echo h($c['name']); ?>" data-iso2="<?php echo h($c['iso2']); ?>"
                    data-iso3="<?php echo h($c['iso3']); ?>" data-phone="<?php echo h($c['phone_country_code']); ?>"
                    data-currency="<?php echo h($c['currency_code']); ?>" data-tz="<?php echo h($c['default_tz']); ?>"
                    data-locale="<?php echo h($c['locale']); ?>" data-datefmt="<?php echo h($c['date_format']); ?>"
                    data-active="<?php echo $isActive ? '1' : '0'; ?>" title="View details" aria-label="View">
                    <i class="bi bi-eye"></i>
                  </button>

                  <!-- Edit -->
                  <button type="button" class="btn btn-warning btn-sm btn-action" data-bs-toggle="modal"
                    data-bs-target="#editModal" data-id="<?php echo (int) $c['id']; ?>"
                    data-name="<?php echo h($c['name']); ?>" data-iso2="<?php echo h($c['iso2']); ?>"
                    data-iso3="<?php echo h($c['iso3']); ?>" data-phone="<?php echo h($c['phone_country_code']); ?>"
                    data-currency="<?php echo h($c['currency_code']); ?>" data-tz="<?php echo h($c['default_tz']); ?>"
                    data-locale="<?php echo h($c['locale']); ?>" data-datefmt="<?php echo h($c['date_format']); ?>"
                    data-active="<?php echo $isActive ? '1' : '0'; ?>" title="Edit country" aria-label="Edit">
                  ...
                  <?php if ($canDelete): ?>
                    <button type="button" class="btn btn-danger btn-sm btn-action" data-bs-toggle="modal"
                      ...
                      data-delete-name="<?php echo h($c['name']); ?>" title="Delete" aria-label="Delete">
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header bg-soft">
        <h6 class="modal-title" id="addModalLabel"><i class="bi bi-plus-circle me-2 text-primary"></i>Add New Country
        </h6>
        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <div class="col-md-2">
              ...
              <div class="input-icon">
                ...
                <input type="text" name="iso2" class="form-control form-control-lg" maxlength="2" placeholder="PH"
            ...
            <div class="col-md-2">
              ...
              <div class="input-icon">
                <i class="bi bi-alphabet"></i>
                <input type="text" name="iso3" class="form-control form-control-lg" maxlength="3" placeholder="PHL"
            ...
            <div class="col-md-3">
              ...
              <div class="input-icon">
                ...
                <input type="text" name="currency_code" class="form-control form-control-lg" maxlength="3"
                  placeholder="PHP" pattern="[A-Za-z]{3}">
              </div>
          ...
          <div class="d-flex align-items-center gap-2">
            ...
            <small class="text-muted">
              A default Business Unit will be created automatically (e.g., <span class="mono">SMC-PH</span> / <span
                class="mono">SMC Philippines</span>).
            </small>
          </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <dl class="row mb-0">
          ...
          <dt class="col-sm-4">ISO2</dt>
          <dd class="col-sm-8" id="v_iso2"></dd>
          <dt class="col-sm-4">ISO3</dt>
          <dd class="col-sm-8" id="v_iso3"></dd>
          <dt class="col-sm-4">Timezone</dt>
          <dd class="col-sm-8" id="v_tz"></dd>
          <dt class="col-sm-4">Phone Code</dt>
          <dd class="col-sm-8" id="v_phone"></dd>
          <dt class="col-sm-4">Currency</dt>
          <dd class="col-sm-8" id="v_currency"></dd>
          <dt class="col-sm-4">Locale</dt>
          <dd class="col-sm-8" id="v_locale"></dd>
          <dt class="col-sm-4">Date Format</dt>
          <dd class="col-sm-8" id="v_datefmt"></dd>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header bg-soft">
        <h6 class="modal-title" id="editModalLabel"><i class="bi bi-pencil-square me-2 text-warning"></i>Edit Country
        </h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <div class="col-md-2">
              ...
              <div class="input-icon">
                <i class="bi bi-alphabet"></i>
                <input type="text" name="iso2" id="e_iso2" class="form-control form-control-lg" maxlength="2"
                  pattern="[A-Za-z]{2}" required>
              </div>
            </div>
            ...
            <div class="col-md-2">
              ...
              <div class="input-icon">
                <i class="bi bi-alphabet"></i>
                <input type="text" name="iso3" id="e_iso3" class="form-control form-control-lg" maxlength="3"
                  pattern="[A-Za-z]{3}" required>
              </div>
            </div>
            ...
            <div class="col-md-3">
              ...
              <div class="input-icon">
                <i class="bi bi-cash-coin"></i>
                <input type="text" name="currency_code" id="e_currency" class="form-control form-control-lg"
...
<div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-labelledby="confirmDeleteLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p class="mb-0">Are you sure you want to permanently delete <strong id="delTargetName">this country</strong>?
      ...
      (function () {
        ...
        const open = () => {
          if (mode === 'add') {
            ...
            if (!f) return;
            const map = {
              id: 'e_id', name: 'e_name', iso2: 'e_iso2', iso3: 'e_iso3', default_tz: 'e_tz',
